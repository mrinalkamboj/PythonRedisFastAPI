...
async def ada_visual_sf(banner:str,weeks:List[str]=Query(None)):
    ...
    ## For a given tracking id, set the status of each week as "pending"
...
async def ada_visual_sf_execute(banner:str,tid:str,qid:str,status=starting):
    ...
    if not current_executing:
        await async_redis_wrapper(set_redis_key,banner+current_executing_suffix,0)
        ## Being the first query set the status of Executing and Proceed
        query_status = executing
    else:
        current_executing = await async_redis_wrapper(incr_redis_key,banner+current_executing_suffix)
...
async def start_pending_task(banner:str,tid:str):
    ...
    if qid is not None:
        ## Execute and Await the Pending Twasks
        await pendingtask
